    ...
    request.on("end", () => {
        try {
            ...
            if (request.url === "/email") {
                sendMail(
                    JSON.parse(data),
                    (data) => sendOK(response, data),
                    (e) => sendBadRequest(response, e.message)
                )
            } else {
                handleQuery(
                    connectionPool,
...
function sendMail(data, successCallback, errorCallback) {
    const mailOptions = {
        from: `"FYS" <${users.email.user}>`,
        to: data.to,
    ...
    transporter.sendMail(mailOptions, (err, info) => {
        return err ? errorCallback(err) : successCallback(info);
    })
}